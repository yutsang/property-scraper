# LeasingHub Comprehensive Scraper

A comprehensive scraper for LeasingHub.com that collects building information from all property types (office, shop, industrial) and their detailed information.
...
python run_leasinghub_scraper.py

# Advanced way - run the main scraper directly
python leasinghub_comprehensive_scraper.py --mode both
```
...
- Scrapes building lists from:
  - 🏢 **Office buildings**: `https://www.leasinghub.com/office/buildings`
  - 🏪 **Shop buildings**: `https://www.leasinghub.com/shop/buildings`
  - 🏭 **Industrial buildings**: `https://www.leasinghub.com/industrial/buildings`
...
python run_leasinghub_scraper.py --buildings-only

# Only scrape details (requires existing building list)
python run_leasinghub_scraper.py --details-only

# Scrape specific property types only
python run_leasinghub_scraper.py --property-types office shop

# Check system status
python run_leasinghub_scraper.py --status

# Check if requirements are installed
python run_leasinghub_scraper.py --check
```
...
# Only building lists
python leasinghub_comprehensive_scraper.py --mode buildings

# Only details
python leasinghub_comprehensive_scraper.py --mode details

# Specific property types
python leasinghub_comprehensive_scraper.py --mode both --property-types office industrial

# Batch detail scraping (useful for resuming)
python leasinghub_comprehensive_scraper.py --mode details --start-idx 100 --batch-size 50
```

## 📁 Output Files

### Building Lists
- `leasinghub_all_buildings.csv` - Combined list from all property types
- `leasinghub_office_buildings.csv` - Office buildings only
- `leasinghub_shop_buildings.csv` - Shop buildings only  
- `leasinghub_industrial_buildings.csv` - Industrial buildings only

### Building Details
- `leasinghub_details_clean_YYYYMMDD_HHMMSS.csv` - Successfully scraped details
- `leasinghub_details_all_YYYYMMDD_HHMMSS.csv` - All attempts (including failures)
...
python run_leasinghub_scraper.py --details-only --start-idx 150

# Process in smaller batches
python run_leasinghub_scraper.py --details-only --start-idx 0 --batch-size 100
```
...
1. **Optimize for Your System**
   ...
   python run_leasinghub_scraper.py --details-only --batch-size 25
   
   # Larger batches for faster systems
   python run_leasinghub_scraper.py --details-only --batch-size 100
   ```
...
2. **Resume Failed Scraping**
   ...
   python run_leasinghub_scraper.py --details-only --start-idx LAST_INDEX
   ```
...
head -20 leasinghub_all_buildings.csv

# Step 3: Scrape details in batches
python run_leasinghub_scraper.py --details-only --batch-size 50
```

### Specific Property Types
```bash
# Only scrape office buildings
python run_leasinghub_scraper.py --property-types office

# Only scrape retail properties
python run_leasinghub_scraper.py --property-types shop

# Scrape office and industrial only
python run_leasinghub_scraper.py --property-types office industrial
```
...
1. Check the status: `python run_leasinghub_scraper.py --status`
2. Verify requirements: `python run_leasinghub_scraper.py --check`
