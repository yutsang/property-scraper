#!/usr/bin/env python3
"""
Simple script to run the Source C Comprehensive Scraper

This script provides easy-to-use functions for scraping Source C data:
...
Usage examples:
    python run_source_c_scraper.py                    # Run complete scraping
    python run_source_c_scraper.py --buildings-only   # Just scrape building lists
    python run_source_c_scraper.py --details-only     # Just scrape details (requires existing building list)
"""
...
def scrape_buildings_only():
    ...
    cmd = "python source_c_comprehensive_scraper.py --mode buildings --property-types office shop industrial"
...
def scrape_details_only(start_idx=0, batch_size=None):
    ...
    if not os.path.exists('source_c_all_buildings.csv'):
        print("❌ Building list not found! Run buildings scraping first.")
        return False
    
    cmd = f"python source_c_comprehensive_scraper.py --mode details --start-idx {start_idx}"
    if batch_size:
...
def scrape_complete():
    ...
    cmd = "python source_c_comprehensive_scraper.py --property-types office shop industrial"
    return run_command(cmd)
...
def scrape_specific_property_types(property_types):
    ...
    cmd = f"python source_c_comprehensive_scraper.py --mode both --property-types {property_args}"
    return run_command(cmd)
...
def show_status():
    ...
    files_info = [
        ('source_c_all_buildings.csv', 'Combined building list'),
        ('source_c_office_buildings.csv', 'Office buildings'),
        ('source_c_shop_buildings.csv', 'Shop buildings'),
        ('source_c_industrial_buildings.csv', 'Industrial buildings'),
    ]
    ...
    detail_files = [f for f in os.listdir('.') if f.startswith('source_c_details_') and f.endswith('.csv')]
    if detail_files:
...
def main():
    parser = argparse.ArgumentParser(description='Easy Source C Scraper Runner')
